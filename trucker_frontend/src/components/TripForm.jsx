...
const TripForm = ({route, method, trip}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Validation
        if (!pickup_location || !dropoff_location || !current_location || !home_operating_center || 
            !start_date || !end_date || !status || !vehicle_no || !trailer_no) {
        ...
        try {
            if (method === "new") {
                ...
                const res = await api.post(route, {
                    pickup_location, dropoff_location, current_location, home_operating_center, start_date, end_date, status, vehicle_no, trailer_no
                });
                console.log(res);
                if (res.status === 201) {
    ...
    return (
        ...
        <Form onSubmit={handleSubmit}>
          <Card
            ...
            bg='dark'
            style={{ boxShadow: 'rgb(157, 154, 154) 0px 5px 15px', maxWidth: '100%', maxHeight: '100%' }}
          >
            <Container className="align-items-center" style={{ fontSize: '0.7rem' }}>
              ...
              <Row xs={3} lg={4}>
                ...
                <Col>
                    ...
                    <Form.Select id="status" defaultValue={status} onChange={(e) => setStatus(e.target.value)}>
                        <option value=""></option>
              ...
              <Col>
                  <Button 
                    ...
                    variant="dark" 
                    disabled={
                      !pickup_location || !dropoff_location || !current_location || 
                      !home_operating_center || !start_date || !end_date || 
                      !status || !vehicle_no || !trailer_no
                    }
                  >
                    {title}
                  </Button>
